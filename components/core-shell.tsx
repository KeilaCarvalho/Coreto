...
import { BarChart3, Banknote, Boxes, ClipboardList, Menu, PackageCheck, Shirt, Users } from "lucide-react";
import Link from "next/link";
import { Sheet, SheetClose, SheetContent, SheetDescription, SheetHeader, SheetTitle, SheetTrigger } from "@/components/ui/sheet";

const links = [
  { href: "/", label: "Recebimento", icon: PackageCheck },
  { href: "/estoque-precificacao", label: "Estoque & Precificação", icon: Boxes },
  { href: "/retiradas", label: "Retiradas", icon: Shirt },
  { href: "/folha-comissao", label: "Folha & Comissão", icon: Users },
  { href: "/fluxo-financeiro", label: "Fluxo Financeiro", icon: Banknote },
  { href: "/vendas-comparativos", label: "Vendas & Comparativos", icon: BarChart3 },
  { href: "/analise-financeira", label: "Análise Financeira", icon: ClipboardList },
];

function Brand() {
  return <div><div className="text-[28px] font-black tracking-[-.04em]">CORETO</div><div className="text-[10px] font-semibold uppercase tracking-[.28em] text-[#65ddda]">Gestão interna</div></div>;
}

function Navigation({ active, mobile = false }: { active: string; mobile?: boolean }) {
  return <nav className="space-y-1 p-4" aria-label="Navegação principal">{links.map((link) => {
...
export function CoreShell({ active, title, subtitle, children }: { active: string; title: string; subtitle: string; children: React.ReactNode }) {
  return <div className="min-h-screen bg-[#f4f7fa] text-[#17304c]">
    <aside className="fixed inset-y-0 left-0 hidden w-[268px] flex-col bg-[#14375f] text-white lg:flex"><div className="border-b border-white/10 px-7 py-6"><Brand/></div><Navigation active={active}/><div className="mt-auto border-t border-white/10 p-5 text-xs text-blue-100/60">Sistema interno Coreto</div></aside>
