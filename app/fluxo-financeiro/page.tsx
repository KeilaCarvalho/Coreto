...
export default function FinancePage() {
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      const canvas = await html2canvas(report, { scale: 2, backgroundColor: "#f7fafc", useCORS: true, logging: false });
  ...
  return <CoreShell active="/fluxo-financeiro" title="Fluxo Financeiro Diário" subtitle="Financeiro / Painel executivo">
    ...
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
      <label className="flex items-center gap-2"><CalendarDays className="text-[#147bc4]"/><span className="sr-only">Data do relatório</span><Input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="w-44 bg-white"/></label>
      <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => saveAll()} disabled={saving}><Save/>{saving ? "Salvando..." : "Salvar painel"}</Button><Button className="bg-[#14375f] hover:bg-[#0f2948]" onClick={downloadPdf} disabled={creatingPdf || saving}><Download/>{creatingPdf ? "Gerando PDF..." : "Baixar novo relatório em PDF"}</Button></div>
    </div>
...
function PdfReport({ ref, date, previousDateLabel, balanceTotal, monthTotal, dayTotal, accountsPayable, accountsPaid, receivableTotal, bankBalances, monthPayments, dayPayments, receivableEntries }: { ref: React.Ref<HTMLDivElement>; date: string; previousDateLabel: string; balanceTotal: number; monthTotal: number; dayTotal: number; accountsPayable: number; accountsPaid: number; receivableTotal: number; bankBalances: Line[]; monthPayments: Line[]; dayPayments: Line[]; receivableEntries: Line[] }) {
  ...
  const reportStyle: React.CSSProperties = { position: "fixed", left: "-10000px", top: 0, width: 794, height: 1123, padding: "38px 38px 28px", boxSizing: "border-box", overflow: "hidden", background: "#ffffff", color: "#11243b", fontFamily: 'Arial, "Helvetica Neue", sans-serif' };
  return <div ref={ref} aria-hidden="true" style={reportStyle}>
    <div style={{ height: 72, display: "flex", alignItems: "center" }}>
      ...
      <div style={{ marginLeft: 15, flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, color: "#008b82", fontSize: 9, fontWeight: 700, letterSpacing: 1.5 }}><i style={{ width: 5, height: 5, borderRadius: 99, background: "#008b82" }}/>CORETO</div>
        <div style={{ color: "#102039", fontWeight: 700, fontSize: 25, lineHeight: 1.05, marginTop: 6 }}>Fluxo Financeiro Diário</div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#20a99f", fontWeight: 700, fontSize: 8, letterSpacing: .55, marginTop: 6 }}><i style={{ width: 4, height: 4, borderRadius: 99, background: "#20c9c5" }}/>RELATÓRIO FINANCEIRO DIÁRIO</div>
      </div>
      <div style={{ width: 130, textAlign: "right", flexShrink: 0 }}><div style={{ color: "#20364f", fontWeight: 700, fontSize: 9, letterSpacing: .55 }}>REFERÊNCIA</div><div style={{ color: "#102039", fontWeight: 700, fontSize: 16, marginTop: 6 }}>{displayDate}</div><div style={{ color: "#008b82", fontSize: 8, marginTop: 4 }}>{weekday}</div></div>
    </div>
    <div style={{ height: 1, background: "#a9c8ca", marginTop: 13, marginBottom: 24 }}/>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
      <PdfSummaryCard label="DISPONIBILIDADE TOTAL" value={balanceTotal} color="#008b82"/>
      <PdfSummaryCard label="CONTAS PAGAS NO MÊS" value={accountsPaid} color="#10b981"/>
      <PdfSummaryCard label="CONTAS A PAGAR AINDA" value={accountsPayable} color="#e88700"/>
      <PdfSummaryCard label="VENDAS DO MÊS" value={monthTotal} color="#102039"/>
      <PdfSummaryCard label="VENDAS DO DIA ANTERIOR" sublabel={previousDateLabel} value={dayTotal} color="#102039"/>
      <PdfSummaryCard label="TOTAL A RECEBER" value={receivableTotal} color="#00a995"/>
      {receivableCards.map((item, index) => <PdfSummaryCard key={index} label={(item.name || `RECEBIMENTO ${index + 1}`).toUpperCase()} value={parseCurrency(item.value)} color="#00a995"/>) }
    </div>
...
function PdfSummaryCard({ label, sublabel, value, color }: { label: string; sublabel?: string; value: number; color: string }) { return <div style={{ height: 74, border: "1px solid #c7d8e6", borderRadius: 11, background: "white", padding: "13px 12px", boxSizing: "border-box", boxShadow: "0 2px 6px rgba(20,55,95,.025)" }}><div style={{ color, fontWeight: 700, fontSize: label.length > 22 ? 8.1 : 9, letterSpacing: .7, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{label}</div><div style={{ display: "flex", alignItems: "end", justifyContent: "space-between", gap: 6, marginTop: 10 }}><div style={{ color: "#102039", fontWeight: 700, fontSize: 17, whiteSpace: "nowrap" }}>{money(value)}</div>{sublabel && <div style={{ color: "#718398", fontSize: 7.5, whiteSpace: "nowrap", paddingBottom: 2 }}>{sublabel}</div>}</div></div>; }
function PdfListBlock({ eyebrow, title, items }: { eyebrow: string; title: string; items: Line[] }) {
  ...
  return <div style={{ height: 276, border: "1px solid #c7d8e6", borderRadius: 12, background: "white", padding: "15px 13px", boxSizing: "border-box" }}>
    <div style={{ color: "#00a28f", fontSize: 8.5, fontWeight: 700, letterSpacing: .65 }}>{eyebrow}</div>
    <div style={{ color: "#102039", fontWeight: 700, fontSize: 14, marginTop: 6 }}>{title}</div>
    <div style={{ height: 205, display: "grid", gridTemplateColumns: "128px 1fr", gap: 13, alignItems: "center" }}>
      ...
      <div>{visible.map((item, index) => <div key={index} style={{ minHeight: 30, padding: "6px 0", borderBottom: "1px solid #d7e3e9", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 9, fontSize: 9.5 }}><span style={{ display: "flex", alignItems: "center", gap: 7, minWidth: 0, color: "#102039" }}><i style={{ width: 8, height: 8, borderRadius: 99, background: pdfColors[index % pdfColors.length], flexShrink: 0 }}/><span>{item.name || "Sem identificação"}</span></span><b style={{ color: "#008b82", whiteSpace: "nowrap" }}>{money(parseCurrency(item.value))}</b></div>)}<div style={{ paddingTop: 9, display: "flex", justifyContent: "space-between", color: "#102039", fontSize: 10.5, fontWeight: 700 }}><span>Total</span><span>{money(total)}</span></div></div>
    </div>
  </div>;
}
...
function TotalBar({ label, value }: { label: string; value: number }) { return <div className="mt-4 flex items-center justify-between rounded-xl bg-[#14375f] p-4 text-white"><span className="text-sm">{label}</span><b className="text-xl text-[#65ddda]">{money(value)}</b></div>; }
function Metric({ label, value, note, icon, details = [], tone = "aqua" }: { label: string; value: string; note?: string; icon: React.ReactNode; details?: StoredLine[]; tone?: "aqua" | "blue" | "orange" | "red" }) { const accent = tone === "orange" ? "bg-[#f59e0b]" : tone === "red" ? "bg-red-400" : tone === "blue" ? "bg-[#4b67be]" : "bg-[#20c9c5]"; const surface = tone === "orange" ? "bg-[#fffaf0]" : tone === "red" ? "bg-red-50" : tone === "blue" ? "bg-[#f7f7ff]" : "bg-[#f7fcfd]"; return <div className={`relative overflow-hidden rounded-xl border border-[#d9e6ee] p-4 shadow-sm ${surface}`}><div className={`absolute inset-y-0 right-0 w-1 ${accent}`}/><div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="text-sm font-medium text-[#708398]">{label}</p><p className="mt-1.5 text-xl font-black text-[#14375f] sm:text-2xl">{value}</p>{note && <p className="mt-1 text-xs text-[#708398]">{note}</p>}</div><div className="rounded-lg bg-white/80 p-2.5 text-[#147bc4] shadow-sm [&>svg]:h-5 [&>svg]:w-5">{icon}</div></div>{details.length > 0 && <div className="mt-3 space-y-1 rounded-lg bg-white/75 p-2.5">{details.slice(0, 3).map((item, index) => <div key={`${item.name}-${index}`} className="flex items-center justify-between gap-3 text-xs"><span className="truncate text-[#587086]">{item.name}</span><b className="shrink-0 text-[#14375f]">{money(item.value)}</b></div>)}</div>}</div>; }
